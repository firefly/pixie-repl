...
exports.DeviceEsp32c3 = exports.Magic = void 0;
...
const esp32_c3_js_1 = require("./stubs/esp32-c3.js");
exports.Magic = [0x6921506f, 0x1b31506f, 0x4881606f, 0x4361606f];
...
class DeviceEsp32c3 extends device_js_1.Device {
    async connect() {
        const magic = await super.connect();
        (0, errors_js_1.assert)(exports.Magic.indexOf(magic) >= 0, `invalid magic number: 0x${magic.toString(16)}`, {
            magic
        });
        return magic;
    }
    async _getStub() { return esp32_c3_js_1.Stub; }
    ...
    */
    async _spiFlashCommand(command, data, responseBits) {
        (0, errors_js_1.assert)(responseBits <= 32, "max SPI response length is 32 bits", {
            length: responseBits
        });
        (0, errors_js_1.assert)(data.length <= 64, "max SPI request length is 64 bytes", {
            length: data.length
        });
        ...
        }
        {
            ...
            if (data.length === 0) {
                ...
                if (data.length % 4 != 0) {
                    ...
                    data = (0, data_js_1.concat)([data, padding]);
                }
                // TODO: This also looks wrong; like it stops short?
                for (let i = 0; i < data.length - 4; i += 4) {
                    await this._writeSpiRegister(reg, (0, data_js_1.fromLeBytes)(data.slice(i, i + 4)));
                    reg += 4;
                }
            }
        }
        ...
        for (let i = 0; i < 11; i++) {
            ...
            (0, errors_js_1.assert)(i < 10, "SPI command did not complete in time");
        }
    ...
    async _writeSpiRegister(offset, value, mask, delayUs, delayAfterUs) {
        ...
        const packet = (0, data_js_1.concat)(fields.map((v) => (0, data_js_1.toLeBytes)(v, 4)));
        await this._command(protocol_js_1.CMD_WRITE_REG, packet);
    }
    async getDeviceInfo() {
        ...
        const flashId = await this._spiFlashCommand(protocol_js_1.CMDSPI_RDID, new Uint8Array(0), 24);
        ...
        if (version === 0) {
            ...
            (0, errors_js_1.assert)(!(version & 1), `invalid version encoding; lsb-set`, {
                reason: "lsb-set", version
            });
            const zeros = (version >> 1) & 0x1f;
            version >>= 6;
            (0, errors_js_1.assert)(zeros === (32 - 6 - countOnes(version)), `invalid version encoding; bad-zero-count`, {
                reason: "bad-zero-count", version
            });
        }
        (0, errors_js_1.assert)(version === 1, `unsupported provision version`, { version });
        const model = await this._readRegister(EFUSE_BASE + 128);
        const serial = await this._readRegister(EFUSE_BASE + 132);
        const modelName = (0, attest_js_1.getModelName)(model);
        return {
    ...
    async getMacAddress() {
        ...
        return [
            (0, data_js_1.hexlify)(mac1 >> 8, 1),
            (0, data_js_1.hexlify)(mac1 & 0xff, 1),
            (0, data_js_1.hexlify)(mac0 >> 24n, 1),
            (0, data_js_1.hexlify)((mac0 >> 16n) & 0xffn, 1),
            (0, data_js_1.hexlify)((mac0 >> 8n) & 0xffn, 1),
            (0, data_js_1.hexlify)(mac0 & 0xffn, 1),
        ].join(":");
    }
}
exports.DeviceEsp32c3 = DeviceEsp32c3;
