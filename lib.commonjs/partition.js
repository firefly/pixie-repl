...
exports.PartitionTable = exports.Partition = void 0;
...
class Partition {
    ...
    constructor(name, type, subtype, offset, size, isReadonly) {
        if ((0, strings_js_1.toUtf8Bytes)(name).length > 16) {
    ...
    get binary() {
        ...
        result.set((0, data_js_1.toLeBytes)(this.offset, 4), 4);
        result.set((0, data_js_1.toLeBytes)(this.size, 4), 8);
        result.set((0, strings_js_1.toUtf8Bytes)(this.name), 12);
        result.set((0, data_js_1.toLeBytes)(flags, 4), 28);
        return result;
    }
}
...
class PartitionTable {
    ...
    addPartition(name, type, subtype, offset, size, isReadonly) {
        ...
        (0, errors_js_1.assert)(this.#flashSize === 0 || offset + size <= this.#flashSize, `partition outside flash range`, {
            name, offset, size, flashSize: this.#flashSize
        });
        if (type === "data") {
            // Check ota data is exactly 0x2000 bytes
            (0, errors_js_1.assert)(subtype != "ota" || size === 0x2000, `ota_data must be 0x2000 bytes`, {
                name, size
            });
            // Check the data partition is 4k bounrary aligned
            (0, errors_js_1.assert)((offset & 0xfff) === 0, `data partition must be aligned on 0x10000 boundary`, {
                name, offset
            });
        }
        if (type === "app") {
            // Check the app partition is 64k boundary aligned
            (0, errors_js_1.assert)((offset & 0xffff) === 0, `app partition must be aligned on 0x10000 boundary`, {
                name, offset
            });
            // Check the size is 4k boundary aligned
            (0, errors_js_1.assert)((size & 0xfff) === 0, `app partition size must be aligned on 0x1000 boundary`, {
                name, size
            });
        }
        // Check the name is unique
        const partition = this.getPartition(name);
        (0, errors_js_1.assert)(!partition, `duplicate partition name: ${name}`, {
            name, partition
        });
        // Check the partition doesn't overlap any other partition
        const partitions = this.partitions;
        for (const partition of partitions) {
            (0, errors_js_1.assert)(offset >= partition.offset + partition.size || offset + size < partition.offset, `overlapping partition: ${name} overlaps ${partition.name}`, {
                name, partition
            });
        }
    ...
    get binary() {
        ...
        const checksum = md5_js_1.Md5.hash(result.slice(0, offset));
    ...
    static from(data, size = 0) {
        (0, errors_js_1.assert)(data.length === 4096, `unexpected data length`, {
            data
        });
        ...
        for (let i = 0; i < data.length; i += 32) {
            ...
            (0, errors_js_1.assert)(d[0] === Magic[0] && d[1] === Magic[1], `invalid magic number`, {
                data, offset: i
            });
            const type = TypeMap[d[2]];
            (0, errors_js_1.assert)(type, `unknown partition type`, { data, type: d[2] });
            const subtype = (type === "app") ? AppTypeMap[d[3]] :
                DataTypeMap[d[3]];
            (0, errors_js_1.assert)(subtype, `unknown partition subtype`, { data, subtype: d[3] });
            const offset = (0, data_js_1.fromLeBytes)(d.slice(4, 8));
            const size = (0, data_js_1.fromLeBytes)(d.slice(8, 12));
            const _name = d.slice(12, 12 + 16);
            let np = 0;
            while (np < _name.length && _name[++np])
                ;
            const name = (0, strings_js_1.toUtf8String)(_name.slice(0, np));
            const flags = (0, data_js_1.fromLeBytes)(d.slice(28));
